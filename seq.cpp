...
void solveSeqKnapsackProblems(vector<int>& capacities, vector<vector<int>>& weights, vector<vector<int>>& values, vector<int>& results) {
    int numProblems = capacities.size();

    for (int i = 0; i < numProblems; i++) {
        results[i] = knapSack(capacities[i], weights[i].data(), values[i].data(), weights[i].size());
    }
}
...
int main() {
    ...
    solveSeqKnapsackProblems(capacities, weights, values, results);
